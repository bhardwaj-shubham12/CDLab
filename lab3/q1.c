...
entry getNextEntry(FILE *fp,char *buf,int lenght)  ///NEXT ENTRY IN TABLE
{
	...
	else
	{
		...
		if(!strcmp(e1.type,"int"))
			...
			e1.size=4;
		/*else if(!strcmp(e1.type,"char"))
		{
			//e1.size=0;
			e1.size=sizeof(buf)/sizeof(buf[0]);
		}*/
		else
			e1.size=-111;
...
void display(listptr head)
{
	...
	while(temp!=NULL)
	{
		//printf("index  name  type  size  scope  No of arg  args  return type");
...
int main()
{
	...
	FILE *fp=fopen("prog.c","r");listptr head=NULL;
	if(!fp)
	{
		...
		else if((isalpha(ca))||ca=='_')			////IDENTIFIERS
		{
			...
			else
			{
				if(!isPreFunc(buf))
				{
					...
					//display(head);
					//printf("%s %s %d %s\n",e1.name,e1.type,e1.narg,e1.arg);
				}
				;
